 ...
 * @param[in] ustr_end 输入的字符串的最后一个位置
 * @param[in] n N-gram中N的取值
 * @param[out] start 词元起始的位置
 * @return 分割出来的词元长度
 */
           ...
           unsigned int n, const UTF32Char **start)
{
    ...
    /* 不断取出最多包含n哥字符的词元，直到遇到不属于索引对象的字符或者到达了字符串的尾部 */
    for (i, p = ustr; i <= n && p < ustr_end
...
create_new_inverted_index(int token_id, int docs_count)
{
    ...
    if (!ii_entry) {
        print_error("cannot allocate memory for ii_entry.");
        return NULL;
    }
...
create_new_postings_list(int document_id)
{
    ...
    if (!pl) {
        print_error("connot allocate memory for pl.");
        return NULL;
    }
...
/**
 * 将构成文档内容的字符串建立倒排列表的集合
 * @param[in] env 存储着应用程序运行环境的结构体
 * @param[in] document_id 文档编号，为0时表示要把查询的关键词作为处理对象
 * @param[in] text 输入的字符串
 * @param[in] text_len 输入的字符串长度
 * @param[in] n N_gram中N的数值
 * @param[in, out] postings 倒排列表的数组。若传入的指针指向了NULL，
 * 则表示要新建一个倒排列表的数组。
 * 若传入的指针指向了之前就已经存在的倒排列表的数组，则表示要添加元素
 * @retval 0 成功
 * @retval -1 失败
*/
                           ...
                           const int n, inverted_index_hash **postings)
{
    ...
    inverted_index_hash *buffer_positings = NULL;
    
    for (; (t_len = ngram_next(t, text_end, n, &t)); t++, position++) {
        /* 检索的时候，忽略掉由于t的长度不足N-grem的最后几个字符构成的词元 */
        if (t_len >= n || document_id) {
            int retval, t_8_size;
            char t_8[n * MAX_UTF8_SIZE];
            
            utf32toutf8(t, t_len, t_8, &t_8_size);
            retval = token_to_positings_list(env, document_id, t_8, t_8_size,
                                             position, &buffer_positings);
            if (retval) { return retval; }
        }
    }
    
    if (*postings) {
        merge_inverted_index(*postings, buffer_positings);
    } else {
        *postings = buffer_positings;
    }
...
/**
 * 为传入的词元创建倒排列表
 * @parag[in] env 存储着应用程序运行环境的结构体
 * @parag[in] document_id 文档编号
 * @parag[in] token 词元
 * @parag[in] token_size 词元的长度（以字节为单位）
 * @parag[in] position 词元出现的位置
 * @parag[in, out] postings 倒排列表的数组
 * @retval 0 成功
 * @retval -1 失败
 */

int token_to_positings_list (wiser_env *env,
